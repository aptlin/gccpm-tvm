...
def train(
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, factor=0.1, patience=5, threshold=1e-2, verbose=True
    )
    if checkpoint_path:
    ...
    for epochId in range(current_epoch, 100):
        ...
        for batch_data in train_loader:
            ...
            for loss_idx in range(len(total_losses)):
                losses.append(
                    l2_loss(stages_output[loss_idx], keypoint_maps, images.shape[0])
                )
        ...
        pck = calc_pckh(
            val_dataset.labels_file_path, predictions_name, eval_num=eval_num
        )
