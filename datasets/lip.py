...
class LipTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(
            os.path.join(
                self._dataset_folder, "TrainVal_images", "train_images", tokens[0]
            ),
            cv2.IMREAD_COLOR,
        )
...
class LipValDataset(Dataset):
    def __init__(self, dataset_folder, num_images=-1):
        ...
        self.labels_file_path = os.path.join(
            self._dataset_folder, "TrainVal_pose_annotations", "lip_val_set.csv"
        )
    ...
    def __getitem__(self, id):
        ...
        image = cv2.imread(
            os.path.join(
                self._dataset_folder, "TrainVal_images", "val_images", tokens[0]
            ),
            cv2.IMREAD_COLOR,
        )
        sample = {"image": image, "file_name": tokens[0]}
        return sample
...
class LipTestDataset(Dataset):
    def __init__(self, dataset_folder):
        ...
        self._names = [
            line.rstrip("\n")
            for line in open(
                os.path.join(self._dataset_folder, "Testing_images", "test_id.txt"), "r"
            )
        ]
    ...
    def __getitem__(self, id):
        ...
        img = cv2.imread(
            os.path.join(self._dataset_folder, "Testing_images", "testing_images", name)
        )
        sample = {"image": img, "file_name": name}
        return sample
