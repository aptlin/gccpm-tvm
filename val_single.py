...
def infer(
    net, img, scales, base_height, stride, img_mean=[128, 128, 128], img_scale=1 / 256
):
    ...
    for ratio in scales_ratios:
        ...
        heatmaps = np.transpose(
            stages_output[-1].squeeze().cpu().data.numpy(), (1, 2, 0)
        )
        heatmaps = cv2.resize(
        ...
        if flip:
            ...
            for r, l in zip(orig_order, flip_order):
                flipped_avg_heatmaps[:, :, r], flipped_avg_heatmaps[:, :, l] = (
                    flipped_avg_heatmaps[:, :, l].copy(),
                    flipped_avg_heatmaps[:, :, r].copy(),
                )
...
if __name__ == "__main__":
    ...
    net = SinglePersonPoseEstimationWithMobileNet(
        num_refinement_stages=5, num_heatmaps=num_heatmaps
    )
