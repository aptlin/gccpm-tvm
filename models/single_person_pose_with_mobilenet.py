...
class UShapedContextBlock(nn.Module):
    def __init__(self, in_channels, mode="bilinear"):
        ...
        self.decoder1 = nn.Sequential(
            conv(in_channels * 3, in_channels * 2), conv(in_channels * 2, in_channels)
        )
    ...
    def forward(self, x):
        ...
        d2 = self.decoder2(
            torch.cat(
                [
                    e1,
                    F.interpolate(
                        e2, size=e1.shape[2:4], mode=self.mode_interpolation,
                    ),
                ],
                1,
            )
        ...
        d1 = self.decoder1(
            torch.cat(
                [
                    x,
                    F.interpolate(d2, size=x.shape[2:4], mode=self.mode_interpolation,),
                ],
                1,
            )
        )
...
class SinglePersonPoseEstimationWithMobileNet(nn.Module):
    def __init__(
        ...
        self.cpm = nn.Sequential(conv(512, 256), conv(256, 128),)
        ...
        for idx in range(num_refinement_stages):
            self.refinement_stages.append(
                RefinementStage(
                    num_channels + num_heatmaps, num_channels, num_heatmaps, mode
                )
            )
    ...
    def forward(self, x):
        ...
        for refinement_stage in self.refinement_stages:
            stages_output.extend(
                refinement_stage(
                    torch.cat([backbone_features, stages_output[-1]], dim=1)
                )
            )

        return torch.cat(stages_output)
