...
class SinglePersonBodyMasking(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() > self._prob:
            ...
            kpt = [
                [
                    center_x - random.randint(1, int(w * self._percentage)),
                    center_y - random.randint(1, int(h * self._percentage)),
                ],
                [
                    center_x + random.randint(1, int(w * self._percentage)),
                    center_y - random.randint(1, int(h * self._percentage)),
...
class SinglePersonFlip(object):
    ...
    def _swap_left_right(self, keypoints):
        keypoints[
            self._right_keypoints_indice + self._left_keypoints_indice
    ...
    def __call__(self, sample):
        ...
        if random.random() > 0.5:
            ...
            if rnd_scale >= 1:  # random crop from upsampled image
                ...
                rnd_offset_y = (sh - h) // 2
                img = scaled_img[
                    rnd_offset_y : rnd_offset_y + h, rnd_offset_x : rnd_offset_x + w
                ]
                rnd_offset_x *= -1
                rnd_offset_y *= -1
        ...
        img = cv2.resize(
            img, dsize=None, fx=scale, fy=scale, interpolation=cv2.INTER_LINEAR
        )
        ...
        padded_img = (
            np.ones((self._crop_y, self._crop_x, 3), dtype=np.uint8) * self._pad
        )
        padded_img[
        ...
        for id in range(len(sample["keypoints"]) // 3):
            if sample["keypoints"][id * 3] == -1:
                continue
            sample["keypoints"][id * 3] = (
                sample["keypoints"][id * 3] * rnd_scale + rnd_offset_x
            ) * scale + offset_x
            sample["keypoints"][id * 3 + 1] = (
                sample["keypoints"][id * 3 + 1] * rnd_scale + rnd_offset_y
            ) * scale + offset_y
...
class RandomScaleRotate(object):
    ...
    def __call__(self, sample):
        ...
        scale = scale * np.clip(
            np.random.normal(loc=1, scale=scale) + 1, 1 - scale, 1 + scale
        )
        rotate = (
            np.clip(np.random.normal(loc=1, scale=rotate), -rotate * 2, rotate * 2)
...
class SinglePersonRandomAffineTransform(object):
    ...
    def __call__(self, sample):
        ...
        trans, _ = self._get_transformation_matrix(
            center, scale, rotate, [self._width, self._height]
        )
        input = cv2.warpAffine(
            sample["image"], trans, (self._width, self._height), flags=cv2.INTER_LINEAR
        )
        sample["trans"] = trans
        if self._mode == "train":
            for id in range(self._num_keypoints):
                sample["keypoints"][3 * id : 3 * id + 2] = self._affine_transform(
                    sample["keypoints"][3 * id : 3 * id + 2], trans
                )
        else:
            sample["rev_trans"] = self._get_transformation_matrix(
                center,
                scale,
                rotate,
                [self._width // self._stride, self._height // self._stride],
            )[1]
        sample["image"] = input
    ...
    def _get_transformation_matrix(self, center, scale, rotate, output_size):
        ...
        points = np.array(
            [center, center + shift_x, center + shift_y], dtype=np.float32
        )
        ...
        rev_trans = cv2.getAffineTransform(
            np.float32(transformed_points), np.float32(points)
        )

        trans = cv2.getAffineTransform(
            np.float32(points), np.float32(transformed_points)
        )
    ...
    def _rotation(point, r):
        r = np.pi * r / 180
        return [
            point[0] * np.cos(r) - point[1] * np.sin(r),
            point[0] * np.sin(r) + point[1] * np.cos(r),
        ]
...
class HalfBodyTransform(object):
    ...
    def __call__(self, sample):
        ...
        do_body_transform = (
            rand <= self._prob
            and np.sum(sample["keypoints"][2:][::3]) > self._num_keypoints
        )
        ...
        for idx in range(len(sample["keypoints"]) // 3):
            if sample["keypoints"][idx * 3 + 2] > 0:
                if idx < 11:
                    upper_points.append(
                        [sample["keypoints"][idx * 3], sample["keypoints"][idx * 3 + 1]]
                    )
                else:
                    lower_points.append(
                        [sample["keypoints"][idx * 3], sample["keypoints"][idx * 3 + 1]]
        ...
        if len(target_points) < 2:
            ...
            center = np.array(
                [
                    target_points[:, 0].sum() / len(target_points),
                    target_points[:, 1].sum() / len(target_points),
                ],
                dtype=np.float32,
            )
