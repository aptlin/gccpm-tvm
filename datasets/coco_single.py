...
def preprocess_bbox(bbox, image):
    ...
    s = np.array([bbox[2] / 200.0, bbox[3] / 200.0], np.float32)
    scale = s * 1.25
...
class CocoSingleTrainDataset(Dataset):
    ...
    def __init__(
        ...
        self._heatmap_size = [
            self._input_size_img[0] // self._stride[0],
            self._input_size_img[1] // self._stride[1],
        ]
        with open(
            os.path.join(
                self._dataset_folder,
                "annotations",
                "person_keypoints_train2017_converted.json",
            )
        ) as f:
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(
            os.path.join(self._dataset_folder, "train2017", image_path),
            cv2.IMREAD_COLOR,
        )
    ...
    def _generate_keypoint_maps(self, sample):
        ...
        target = np.zeros(
            (len(keypoints) // 3, self._heatmap_size[1], self._heatmap_size[0]),
            dtype=np.float32,
        )
        ...
        for i in range(self._num_keypoints):
            if keypoints[i * 3 + 2] > 0:  # visible
                ...
                rb = [x + eps + 1, y + eps + 1]
                if (
                    lt[0] >= self._heatmap_size[0]
                    or lt[1] >= self._heatmap_size[1]
                    or rb[0] < 0
                    or rb[1] < 0
                ):
                    keypoints[i * 3 + 2] = 0
                ...
                y0 = x0
                gaussian = np.exp(
                    -((grid_x - x0) ** 2 + (grid_y - y0) ** 2) / (2 * self._sigma ** 2)
                )
                lt_heatmap = [max(0, lt[0]), max(0, lt[1])]
                rb_heatmap = [
                    min(rb[0], self._heatmap_size[0]),
                    min(rb[1], self._heatmap_size[1]),
                ]

                lt_gaussian = [max(0, -lt[0]), max(0, -lt[1])]
                rb_gaussian = [
                    min(rb[0], self._heatmap_size[0]) - lt[0],
                    min(rb[1], self._heatmap_size[1]) - lt[1],
                ]

                if keypoints[i * 3 + 1] > 0.5:
                    target[i][
                        lt_heatmap[1] : rb_heatmap[1], lt_heatmap[0] : rb_heatmap[0]
                    ] = gaussian[
                        lt_gaussian[1] : rb_gaussian[1], lt_gaussian[0] : rb_gaussian[0]
                    ]
...
class CocoSingleValDataset(Dataset):
    def __init__(self, dataset_folder, num_images=None, transform=None):
        ...
        with open(
            os.path.join(
                self._dataset_folder, "annotations", "person_keypoints_val2017.json"
            )
        ) as f:
            data = json.load(f)
    ...
    def __getitem__(self, idx):
        image_path = next(
            ...
            if it["id"] == self._annotations[idx]["image_id"]
        )
        image = cv2.imread(
            os.path.join(self._dataset_folder, "val2017", image_path), cv2.IMREAD_COLOR
        )
