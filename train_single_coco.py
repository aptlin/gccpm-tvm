...
def train(
    ...
    dataset = CocoSingleTrainDataset(
        images_folder,
        transform=transforms.Compose(
            [
                ...
                RandomScaleRotate(),
                SinglePersonFlip(
                    left_keypoints_indice=CocoSingleTrainDataset.left_keypoints_indice,
                    right_keypoints_indice=CocoSingleTrainDataset.right_keypoints_indice,
                ),
                SinglePersonRandomAffineTransform(),
                SinglePersonBodyMasking(),
    ...
    net = SinglePersonPoseEstimationWithMobileNet(
        num_refinement_stages, num_heatmaps=dataset._num_keypoints, mode="nearest"
    ).cuda()
    ...
    for epochId in range(current_epoch, 210):
        ...
        for batch_data in train_loader:
            ...
            for loss_idx in range(len(total_losses)):
                losses.append(
                    mse_loss(
                        stages_output[loss_idx],
                        keypoint_maps,
                        batch_data["keypoints"][:, 2::3].view(
                            batch_data["keypoints"].shape[0], -1, 1
                        ),
                    )
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset-folder", type=str, required=True, help="path to dataset folder"
    )
    parser.add_argument(
