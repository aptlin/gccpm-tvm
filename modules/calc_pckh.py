...
def get_normalized_distance(gt, prediction, head_size):
    ...
    for img_id in range(num_images):
        ...
        if current_head_size == 0:
            ...
            distances[img_id, :] = (
                np.linalg.norm(gt[img_id, :, :] - prediction[img_id, :, :], axis=1)
                / current_head_size
            )
...
def compute_pckh(distances, pckh_threshold_range):
    ...
    for kpt_id in range(num_keypoints):
        for threshold_id in range(len(pckh_threshold_range)):
            ...
            pckh[threshold_id, kpt_id] = (
                np.mean(joint_distance[np.where(joint_distance >= 0)] <= threshold)
                * 100
            )
    ...
    for threshold_id in range(len(pckh_threshold_range)):
        ...
        pckh[threshold_id, num_keypoints] = (
            np.mean(joint_distance[np.where(joint_distance >= 0)] <= threshold) * 100
        )
    ...
    for threshold_id in range(len(pckh_threshold_range)):
        ...
        pckh[threshold_id, num_keypoints + 1] = (
            np.mean(joint_distance[np.where(joint_distance >= 0)] <= threshold) * 100
        )
...
def print_output(pckh, method_name):
    template = (
        "{0:10} & {1:6} & {2:6} & {3:6} & {4:6} & {5:6} & {6:6} & {7:6} & {8:6} & {9:6}"
    )
    ...
    pckh = template.format(
        method_name,
        "%1.2f" % ((pckh[8] + pckh[9]) / 2),
